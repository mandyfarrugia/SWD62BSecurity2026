    ...
    public class ExternalLoginModel : PageModel
    {
        ...
        public async Task<IActionResult> OnPostConfirmationAsync(string returnUrl = null)
        {
            ...
            if (ModelState.IsValid)
            {
                ...
                user.FirstName = info.Principal.Claims.FirstOrDefault(claim => claim.Type.Contains("givenname")).Value;
                user.LastName = info.Principal.Claims.FirstOrDefault(claim => claim.Type.Contains("surname")).Value;
                ...
                if (result.Succeeded)
                {
                    ...
                    AsymmetricParameters keys = encryptionTool.GenerateAsymmetricKeys();

                    string encryptionPassword = createdUser.Id;
                    byte[] privateKeyBytes = Encoding.UTF8.GetBytes(keys.PrivateKey);
                    byte[] encryptedPrivateKeyBytes = encryptionTool.SymmetricEncrypt(privateKeyBytes, encryptionPassword);

                    createdUser.PublicKeyXml = keys.PublicKey;
                    createdUser.EncryptedPrivateKey = Convert.ToBase64String(encryptedPrivateKeyBytes);
